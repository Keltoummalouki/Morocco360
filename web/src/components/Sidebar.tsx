...
const ICONS: Record<string, React.ReactNode> = {
  ...
  organizers:<Icon d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0z" />,
  staff:     <Icon d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z" />,
  bookings:  <Icon d="M15 5v2m0 4v2m0 4v2M5 5a2 2 0 00-2 2v3a2 2 0 110 4v3a2 2 0 002 2h14a2 2 0 002-2v-3a2 2 0 110-4V7a2 2 0 00-2-2H5z" />,
  payments:  <Icon d="M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z" />,
  reviews:   <Icon d="M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.783.57-1.838-.196-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.783-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z" />,
...
const NAV_ITEMS: Record<Role, NavItem[]> = {
  ADMIN: [
    ...
    { key: 'organizers', href: '/dashboard/admin/organizers', icon: ICONS.organizers },
    { key: 'staff',      href: '/dashboard/admin/staff',      icon: ICONS.staff      },
    { key: 'bookings',   href: '/dashboard/admin/bookings',   icon: ICONS.bookings   },
    { key: 'payments',   href: '/dashboard/admin/payments',   icon: ICONS.payments   },
    { key: 'reviews',    href: '/dashboard/admin/reviews',    icon: ICONS.reviews    },
    { key: 'settings',   href: '/dashboard/admin/settings',   icon: ICONS.settings   },
  ],
  ORGANIZER: [
  ...
  STAFF: [
    { key: 'overview', href: '/dashboard/staff',        icon: ICONS.overview },
    { key: 'events',   href: '/dashboard/staff/events', icon: ICONS.events   },
  ],
  USER: [
    { key: 'explore',  href: '/user/events',   icon: ICONS.explore },
    { key: 'events',   href: '/user/events',   icon: ICONS.events  },
    { key: 'saved',    href: '/user/saved',    icon: ICONS.saved   },
    { key: 'history',  href: '/user/history',  icon: ICONS.history },
    { key: 'profile',  href: '/user/profile',  icon: ICONS.profile },
  ],
};
...
const ROLE_COLORS: Record<Role, { bg: string; text: string; dot: string }> = {
  ...
  STAFF:     { bg: 'rgba(107,114,128,0.12)', text: '#6B7280', dot: '#6B7280' },
  ...
  const sidebarContent = (
    <aside className="sidebar-premium">
      ...
      <nav style={{ flex: 1, padding: '12px 10px' }}>
        {items.map((item) => {
          // 'overview' href is the dashboard root, a prefix of every sub-page —
          // match it exactly so it isn't always highlighted.
          const active =
            item.key === 'overview'
              ? pathname === item.href
